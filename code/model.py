...
class KGEModel(nn.Module):
    def __init__(self, model_name, nentity, nrelation, hidden_dim, gamma, u,
        ...
        if model_name not in ['TransE', 'DistMult', 'ComplEx', 'RotatE', 'pRotatE', 'ReCS', 'RoCS', 'RoReCS', 'RoAddCS']:
    ...
    def RoReCS(self, head, relation, tail, mode):
        ...
        if mode == 'head-batch':
            ...
            im_rt = re_relation * im_tail - im_relation * re_tail
            score = self.ComplexCosine(re_head, im_head, re_rt, im_rt)
    ...
    def RoAddCS(self, head, relation, tail, mode):
        ...
        if mode == 'head-batch':
            re_rt = re_relation * re_tail + im_relation * im_tail
            im_rt = re_relation * im_tail - im_relation * re_tail
            re_score = torch.cosine_similarity(re_head, re_rt, dim = 2)
            im_score = torch.cosine_similarity(im_head, im_rt, dim = 2)

        else:
            re_hr = re_head * re_relation - im_head * im_relation
            im_hr = re_head * im_relation + im_head * re_relation
            re_score = torch.cosine_similarity(re_hr, re_tail, dim = 2)
            im_score = torch.cosine_similarity(im_hr, im_tail, dim = 2)
    ...
    def ComplexCosine(re_x, im_x, re_y, im_y):
        ...
        # imag_dot = imag_dot.sum(dim = 2)
        norm_x = torch.stack([re_x, im_x], dim = 0)
        norm_x = norm_x.norm(dim = 0)
        norm_x = norm_x.norm(dim = 2)
        norm_y = torch.stack([re_y, im_y], dim = 0)
        norm_y = norm_y.norm(dim = 0)
        norm_y = norm_y.norm(dim = 2)

        score = real_dot / (norm_x * norm_y)
